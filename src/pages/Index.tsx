...
import { LayoutDashboard, ShoppingCart, CheckSquare, Landmark, Package, Menu, X, ChevronRight } from "lucide-react";
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 flex flex-col bg-sidebar text-sidebar-foreground transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* Logo */}
          ...
          {menuItems.map((item) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p
                  className={`text-[10px] truncate ${active === item.id ? "text-white/60" : "text-sidebar-foreground/40"}`}
                >
                  {item.sublabel}
                </p>
              </div>
